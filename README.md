...
- `server/` – ASP.NET Core 8 Web API backed by Entity Framework Core targeting SQL Server.
...
The API connects to SQL Server using the `Default` connection string and enables Swagger UI in development mode at `/swagger`.
...
The ASP.NET API uses the `Default` connection string defined in `appsettings.json`. Update it with the server, database name, and credentials appropriate for your SQL Server instance.
